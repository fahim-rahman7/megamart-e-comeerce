...
import { 
  FiEdit3, FiSave, FiX, FiCamera, FiUser, FiMapPin, 
  FiMail, FiShield, FiShoppingBag, FiPackage, FiTruck, FiCheckCircle, FiClock
} from "react-icons/fi";
...
const Profile = () => {
  ...
  // Derived Data
  const cartItemsCount = cartRes?.cartData?.totalItems || cartRes?.cartData?.items?.length || 0;
  ...
  useEffect(() => {
    if (profileData) {
      ...
      setAddress(typeof profileData.address === "string" ? profileData.address : profileData.address?.street || "");
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container max-w-6xl mx-auto px-4 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Link to="/cart" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center gap-4 hover:border-brand transition group cursor-pointer">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold text-gray-900">{cartItemsCount}</h3>
            </div>
          </Link>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold text-gray-900">{deliveredOrders}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleUpdateProfile} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-full">
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="font-medium text-gray-800 bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">{profileData.fullName}</p>
                  )}
                </div>
                    ...
                    {profileData.address ? (
                      <p className="text-gray-800 leading-relaxed">{
                        typeof profileData.address === "string" 
                          ? profileData.address 
                          : `${profileData.address.street || profileData.address.address}, ${profileData.address.city}, ${profileData.address.postalCode}`
                      }</p>
                    ) : (
                      <p className="italic text-gray-400">No address added yet. Click edit to add one.</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-full">
            ...
            {isOrdersLoading ? (
              <p className="text-sm text-gray-500 animate-pulse text-center py-8">Loading orders...</p>
              ...
              <div className="flex-grow flex flex-col items-center justify-center text-center py-8">
                ...
                <p className="text-sm text-gray-500 font-medium">No orders placed yet.</p>
                <Link to="/shop" className="text-brand text-sm font-semibold hover:underline mt-2 inline-block">Explore Shop</Link>
              </div>
            ) : (
              <div className="flex-grow flex flex-col justify-between">
                <div className="space-y-4">
                  {recentOrders.map((order) => (
                    <div key={order._id} className="p-4 rounded-xl border border-gray-100 hover:border-brand/30 hover:bg-brand/5 transition">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="text-xs text-gray-500 font-medium">Order #{order.orderNumber?.slice(-6) || "N/A"}</p>
                      ...
                      <div className="flex justify-between items-end mt-3 pt-3 border-t border-gray-50">
                        <div>
                          <p className="text-xs text-gray-500 capitalize">{order.items?.length || 0} items • {order.payment?.method || 'N/A'}</p>
                        </div>
                ...
                <div className="pt-6 mt-4 border-t border-gray-100">
                  <Link to="/orders" className="block text-center text-sm font-semibold text-brand hover:text-brand/80 transition bg-brand/5 py-3 rounded-xl w-full">
